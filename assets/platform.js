...
(function (global) {
  ...
  Platform.initHomeworkPage = function (lessonId, lessonTitle) {
    ...
    if (hwList) {
      ...
      // Відновити позначки, якщо учень заходив раніше
      Platform.getProgress(lessonId).then(function (data) {
        // Позначки не зберігаються поштучно, тому просто лишаємо як є —
        // головне, що прогрес (скільки виконано) видно вчителю в панелі.
      });
    }
  };
